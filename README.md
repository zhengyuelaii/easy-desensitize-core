...
- **⚡ 极致性能**：采用 **"局部强引用 + 全局软引用"** 的二级缓存架构，反射元数据解析损耗降低 90% 以上。
- **🔍 深度递归**：完美支持 `List`、`Set`、`Map` 及多层嵌套 Bean 的递归脱敏，自动识别并处理复杂对象图。
- **🛡️ 泛型友好**：解决了 Java **泛型擦除**难题，通过运行时动态探测，精准处理 `Result<T>`、`Page<T>` 等泛型包装类。
...
|:--------------| :--- |:------------| :--- |
| **1,00 条**    | 0.048 ms | **~480 ns** | 极其轻量 |
| **1,000 条**   | 0.447 ms | **~447 ns** | 极致稳定 |
| **10,000 条**  | 5.513 ms | **~551 ns** | 高效处理 |
| **100,000 条** | 68.269 ms | **~682 ns** | 线性伸缩 |

> **数据分析**：处理单条复杂嵌套对象的损耗控制在 **1微秒 ($1\mu s$)** 以内。在处理 10 万级大数据量时，性能呈完美的线性增长，无明显的 GC 抖动或内存泄露。
...
2. **L2 全局缓存 (Global Cache)**：
   - **实现**：`ConcurrentHashMap<Class, SoftReference<List<FieldMeta>>>`
   - **作用**：跨请求复用解析结果。使用 **软引用 (SoftReference)** 包装，当 JVM 内存不足时自动回收，杜绝 OOM 风险。
